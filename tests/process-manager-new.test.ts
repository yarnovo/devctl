...
describe('ProcessManager', () => {
  ...
  beforeEach(() => {
    ...
    memoryFs = container.resolve<IFileSystem>(
      FILE_SYSTEM_TOKEN
    ) as MemoryFileSystem
  ...
  describe('start', () => {
    it('当服务器已经运行时应该显示错误信息', async () => {
      const pidFile = '/project/logs/dev.pid'
      const runningPid = process.pid // 使用当前进程 PID 模拟运行中的服务

      createVirtualFileSystem({
        [pidFile]: runningPid.toString(),
      })

      await processManager.start()

      expect(consoleSpy).toHaveBeenCalledWith(
        `❌ 开发服务器已经在运行中! PID: ${runningPid}`
      )
      expect(consoleSpy).toHaveBeenCalledWith(
